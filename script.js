...
setTimeout(() => {
    ...
    if (typeof elements !== 'undefined' && elements.loadingScreen) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Acadamic Table
    const academicData = [
        { year: "2025-Present", institution: "University of Engineering and Technology", degree: "BS Software Engineering", grade: "Ongoing" },
...
function updateProgress() {
  ...
  const percText = document.getElementById('progressPercent');
  if(fill) fill.style.width = percent + '%';
  if(percText) percText.textContent = percent + '%';
  
  const compEl = document.getElementById('completedTasks');
  const totEl = document.getElementById('totalTasks');
...
if (document.readyState === 'loading') {
  ...
  initProjects();
    ...
    // Contact With me - INTEGRATED WITH BACKEND
        ...
        contactForm.addEventListener('submit', async (e) => {
            ...
            if (isValid) {
                ...
                try {
                    ...
                    alert("Server is offline. Data could not be saved.");
                }
            }
        });
    }
    ...
    document.querySelectorAll('.profile-card, .academic-table, .cv-viewer, .skill-card, .gallery-item, .contact-form').forEach(el => scrollObserver.observe(el));
